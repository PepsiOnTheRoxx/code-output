...
class BronService:
    ...
    def update_bron(self, bron_id, nieuwe_data):
        ...
        if not bron:
            raise BronNietGevondenException(f"Bron with id {bron_id} not found")
        ...
        if 'naam' in nieuwe_data and (nieuwe_data['naam'] is None or str(nieuwe_data['naam']).strip() == ''):
            raise OngeldigeBronDataException("Naam mag niet leeg zijn")

        if 'type' in nieuwe_data and nieuwe_data['type'] != self.METAMODEL_TYPE:
            raise OngeldigeBronDataException(f"Type mag niet gewijzigd worden en moet '{self.METAMODEL_TYPE}' blijven")
        ...
        if updated_bron.get('type') != self.METAMODEL_TYPE or updated_bron.get('element_id') != self.METAMODEL_ELEMENT_ID:
            raise OngeldigeBronDataException("Type of element_id klopt niet met metamodel")
