import pytest
from unittest.mock import patch, MagicMock
from src.services.deletegebruiker import GebruikerService
from src.services.deletegebruiker_exceptions import GebruikerNietGevondenException, VerwijderGebruikerException

def test_delete_gebruiker_success():
    gebruiker_id = 1
    service = GebruikerService()
    with patch.object(service, 'verwijder_gebruiker', return_value=True) as mock_verwijder:
        result = service.verwijder_gebruiker(gebruiker_id)
        assert result is True
        mock_verwijder.assert_called_once_with(gebruiker_id)

def test_delete_gebruiker_not_found():
    gebruiker_id = 42
    service = GebruikerService()
    with patch.object(service, 'verwijder_gebruiker', side_effect=GebruikerNietGevondenException):
        with pytest.raises(GebruikerNietGevondenException):
            service.verwijder_gebruiker(gebruiker_id)

def test_delete_gebruiker_database_error():
    gebruiker_id = 7
    service = GebruikerService()
    with patch.object(service, 'verwijder_gebruiker', side_effect=VerwijderGebruikerException("Fout")):
        with pytest.raises(VerwijderGebruikerException) as exc_info:
            service.verwijder_gebruiker(gebruiker_id)
        assert str(exc_info.value) == "Fout"

def test_delete_gebruiker_calls_external_repository():
    gebruiker_id = 3
    with patch('src.services.deletegebruiker.GebruikerRepository') as mock_repo_cls:
        mock_repo_instance = MagicMock()
        mock_repo_cls.return_value = mock_repo_instance
        service = GebruikerService()
        with patch.object(service, 'verwijder_gebruiker', wraps=service.verwijder_gebruiker) as real_method:
            try:
                service.verwijder_gebruiker(gebruiker_id)
            except Exception:
                pass  # In geval van niet-geïmplementeerd
            assert mock_repo_cls.called