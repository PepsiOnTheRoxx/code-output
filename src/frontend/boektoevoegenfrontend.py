...
def boek_toevoegen():
    if request.method == 'POST':
        ...
        conn = get_db()
        cur = conn.cursor()
        cur.execute('''INSERT INTO boeken (auteur, beschrijving, isbn, publicatiedatum, kaft_foto_url, is_uitgeleend, uitgeleend_datum, uitgeleend_max_tot)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                    (auteur, beschrijving, isbn, publicatiedatum, kaft_foto_url, is_uitgeleend, uitgeleend_datum, uitgeleend_max_tot))
        conn.commit()
        boek_id = cur.lastrowid
        conn.close()
        return redirect(url_for('boekdetailfrontend.boek_detail', boek_id=boek_id))
