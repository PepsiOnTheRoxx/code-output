...
def catalogus_overzicht():
    conn = get_db()
    boeken = conn.execute('SELECT rowid, * FROM boeken').fetchall()
    conn.close()
...
def boek_detail(boek_id):
    conn = get_db()
    boek = conn.execute('SELECT rowid, * FROM boeken WHERE rowid=?', (boek_id,)).fetchone()
    conn.close()
...
def boek_toevoegen():
    if request.method == 'POST':
        ...
        conn = get_db()
        conn.execute('INSERT INTO boeken (auteur, kaft_foto_url, is_uitgeleend) VALUES (?, ?, ?)',
                     (auteur, kaft_foto_url, is_uitgeleend))
        conn.commit()
...
def boek_aanpassen(boek_id):
    ...
    if request.method == 'POST':
        ...
        kaft_foto_url = request.form['kaft_foto_url']
        is_uitgeleend = 1 if request.form.get('is_uitgeleend') else 0
        conn.execute('UPDATE boeken SET auteur=?, kaft_foto_url=?, is_uitgeleend=? WHERE rowid=?',
                     (auteur, kaft_foto_url, is_uitgeleend, boek_id))
        conn.commit()
        conn.close()
        return redirect(url_for('catalogusfrontend.boek_detail', boek_id=boek_id))
    boek = conn.execute('SELECT rowid, * FROM boeken WHERE rowid=?', (boek_id,)).fetchone()
