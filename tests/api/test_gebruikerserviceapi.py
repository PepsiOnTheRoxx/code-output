...
def test_get_gebruiker_success():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        ...
        mock_service_instance.get_by_id.return_value = gebruiker_data
...
def test_get_gebruiker_not_found():
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.get_by_id.side_effect = GebruikerNotFoundException("Niet gevonden")
...
def test_create_gebruiker_success():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.create.return_value = opgeslagen_gebruiker
        result = create_gebruiker(nieuwe_gebruiker)
        assert result == opgeslagen_gebruiker
        mock_service_instance.create.assert_called_once_with(nieuwe_gebruiker)

def test_create_gebruiker_already_exists():
    nieuwe_gebruiker = {"naam": "Klaas", "email": "klaas@example.com"}
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.create.side_effect = GebruikerAlreadyExistsException("Bestaat al")
...
def test_create_gebruiker_invalid():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.create.side_effect = GebruikerValidationException("Ongeldige invoer")
...
def test_update_gebruiker_success():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.update.return_value = updated_gebruiker
        result = update_gebruiker(gebruiker_id, wijziging)
        assert result == updated_gebruiker
        mock_service_instance.update.assert_called_once_with(gebruiker_id, wijziging)
...
def test_update_gebruiker_not_found():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.update.side_effect = GebruikerNotFoundException("Niet gevonden")
        with pytest.raises(GebruikerNotFoundException):
...
def test_update_gebruiker_validation():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.update.side_effect = GebruikerValidationException("Fout")
        with pytest.raises(GebruikerValidationException):
            update_gebruiker(gebruiker_id, wijziging)
...
def test_delete_gebruiker_success():
    ...
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        ...
        mock_service_instance.delete.assert_called_once_with(gebruiker_id)

def test_delete_gebruiker_not_found():
    gebruiker_id = 999
    with patch("src.api.gebruikerserviceapi.GebruikerService") as MockService:
        mock_service_instance = MockService.return_value
        mock_service_instance.delete.side_effect = GebruikerNotFoundException("Niet gevonden")
        with pytest.raises(GebruikerNotFoundException):
