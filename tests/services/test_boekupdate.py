...
def test_update_boek_onbekend_veld_wordt_genegeerd(boek_service, mock_boek_repo):
    bestaand_boek = MagicMock()
    bestaand_boek.titel = 'Oud Titel'
    bestaand_boek.auteur = 'Oud Auteur'
    mock_boek_repo.get_boek_by_id.return_value = bestaand_boek
    nieuwe_data = {'titel': 'Nieuw Titel', 'jaar': 2024}  # 'jaar' is geen geldig veld, moet genegeerd worden
    boek_service.update_boek(1, nieuwe_data)
    assert bestaand_boek.titel == 'Nieuw Titel'
    assert bestaand_boek.auteur == 'Oud Auteur'
    mock_boek_repo.save_boek.assert_called_once_with(bestaand_boek)
